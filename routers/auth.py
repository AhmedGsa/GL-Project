...
def register_avocat(avocatRegisterSchema: AvocatRegisterSchema, db: Session = Depends(get_db)):
    userExists = user.get_by_email(db, avocatRegisterSchema.email)
    if userExists:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Email already used")
    newUser = user.create(db, avocatRegisterSchema)
    avocat.create(db, {"userId": newUser.id, **avocatRegisterSchema.model_dump()})
    token = JWT.create_token({"id": newUser.id, "email": newUser.email})
    return {"token": token}